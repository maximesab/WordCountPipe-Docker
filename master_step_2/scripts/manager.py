...
class Cache ():
    ...
    def __init__(self, path, arg=None):
        ...
        self.sshcommand = "sshpass -p" + os.getenv('PASSWORD','XXXXXXXXXXX') + " ssh -o 'StrictHostKeyChecking no' " +os.getenv('HOST_USER','maxime') +"@172.17.0.1 "
    ...
    def scale(self):
        number_of_container = int(os.popen(self.sshcommand +" docker ps | grep -e 'wordcountpipe_combine' | wc -l " ).read())
        if number_of_container <= len(self.dirs):
            print('number_of_container:' , number_of_container)
            os.system(self.sshcommand +' docker-compose -f '+ '/home/maxime/Desktop/WordCountPipe/docker-compose.yml'   + ' scale combine'  + '=' +str(number_of_container + 1) ) 
        print('done')
    ...
    def update(self, event):
        ...
        if event.get('type') == "DELETE" or  event.get('type') == "REMOVE":            
            ...
            self.count_file()
            self.sub_info()
            print('clean up suppose to start' +'/n')
            self.clean_up(event)
