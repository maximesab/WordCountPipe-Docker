...
class Cache ():
    ...
    def update(self, event):
        print ("event type is create or move to", event.get('type') == "CREATE" or event.get('type') == "MOVED_TO")
            ...
            if files in self.files:
                for directory_name in self.dirs:
                    if self.dirs.get(directory_name).file_count < self.file_limit:
                        file = random.choice(self.files.keys())
                        os.rename(file, os.path.join(directory_name,os.path.basename(file)))
                        break
        except:
